...
「60秒魔法ゲーム」の試作です。手（またはマウス）で線を描き、声で唱えた言葉から魔法を作り、騎士へ放ちます。設計仕様の最初の一回（24秒）だけを遊べます。防御の回、最後の魔法、魔導書、QR、展示の連続運転は次の段階です。
...
- 24秒の中で何がいつ起きるか、Jevと音声認識の細かい動き：[docs/動きの詳細.md](docs/動きの詳細.md)
...
[http://localhost:5173](http://localhost:5173) を開きます。「マウスで試す」を選び「魔法をつくる」を押すと遊べます。3秒の合図のあと24秒が始まります。押したまま動かすと線が残り、画面右側の欄に声の代わりの言葉を入れられます。
...
| 場所 | 中身 |
| --- | --- |
| `src/game/` | 時計、手の軌跡、発話の更新、魔法の決定、詠唱辞書、確認用の記録 |
...
| 最後の声を待つ時間（1.4秒）、待ちの打ち切り（15.4秒）、Jevの打ち切り（15.9秒）、確定（16秒） | `src/game/session.ts` の先頭 |
