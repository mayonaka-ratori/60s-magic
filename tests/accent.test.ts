...
describe('二つ目の属性を飾り色にする',()=>{
  ...
  it('属性語が一つだけ、同じ属性語の繰り返し、属性語なしでは飾り色が付かない',()=>{
    // 主属性を見ない行は null。
    const table:Array<[string,string|null]>=[['氷よ','ice'],['炎よ、炎の球となれ','fire'],['静かな力よ',null]];
    for(const [text,element] of table){
      const r=makeRecipe(state(text));
      if(element!==null)expect(r.element,text).toBe(element);
      expect(r.accent,text).toBe(null);
    }
  });
...
describe('二属性の合わせ方の三型',()=>{
  // 決め方。熱と冷（炎と氷）、明と暗（光と闇）の対どうしは相反するので「増幅」。
  // 雷が荒れる組み合わせと炎＋風は「爆発」。残りは並び立つので「持続」。
  // 組み合わせを全部は並べず、型ごとに一組ずつ見る。
  const table:Array<[string,string,string]>=[['fire','ice','amplify'],['fire','lightning','burst'],['ice','wind','sustain']];
  it('型ごとの組で型が決まり、前後を入れ替えても同じ',()=>{
