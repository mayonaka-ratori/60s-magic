...
describe('蓄積の粒は画面の端から生まれる', () => {
  ...
  it('粒の上限を超えず、控えめモードでは3分の1になる', () => {
    ...
    const dropped = setup(); dropped.pool.at = 16.5; drawCharge(dropped.frame(16.5, .5));
...
describe('床の明かりと魔法陣は術式の範囲に合わせる', () => {
  ...
  it('範囲の下端の中央に属性の色の楕円を敷き、蓄積が進むほど明るく、濃さは0.35まで', () => {
    const early = setup(); drawCharge(early.frame(14.6, 1 / 60));
    const late = setup(); drawCharge(late.frame(16.9, 1 / 60));
    ...
    // 入力の量を重ねても上限は超えない。
    const full = setup({ live: { words: [], amount: 1, voice: 0, rings: 0 } }); drawCharge(full.frame(16.99, 1 / 60));
  ...
  it('中心の魔法陣は範囲の横幅の半分まで広がり、画面の高さの30%を超えない', () => {
    const s = setup(); drawCharge(s.frame(16.99, 1 / 60));
    ...
    const wide = setup({ extent: { x: 40, y: 200, width: 1200, height: 300 } }); drawCharge(wide.frame(16.99, 1 / 60));
    expect(Math.max(...arcRadii(wide.log))).toBeLessThanOrEqual(H * .3 * 1.22 + .01);
    // 小さな術式では今までの大きさ（36 + 派手さ×12）を保つ。
    const tiny = setup({ extent: { x: 690, y: 440, width: 20, height: 20 } }); drawCharge(tiny.frame(16.99, 1 / 60));
    expect(Math.max(...arcRadii(tiny.log))).toBeGreaterThan(36 + 1.5 * 12);
  });
  it('入力の量が多いときだけ、光点の足元にも小さな陣が出る', () => {
    const few = setup(); drawCharge(few.frame(16.5, 1 / 60));
    const many = setup({ live: { words: [], amount: 1, voice: 0, rings: 0 } }); drawCharge(many.frame(16.5, 1 / 60));
  ...
  it('地面から昇る光の筋は、範囲の横幅いっぱいに散らばる', () => {
    const s = setup(); drawCharge(s.frame(16, 1 / 60));
  ...
  it('控えめの設定では魔法陣を出さないが、床の明かりは出る', () => {
    const s = setup({ preset: presets.calm }); drawCharge(s.frame(16.5, 1 / 60));
