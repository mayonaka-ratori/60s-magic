...
export class LocalSpeech {
  ...
  start() {
    ...
    this.status={...this.status,state:'loading',message:'このPCの音声認識を準備しています'};
    ...
    createInterface({input:child.stdout}).on('line',line=>{
      ...
      if(message.type==='result'&&this.active?.id===message.id) {
        ...
        if(!job)return;
        if(message.error||typeof message.text!=='string'||message.text.length>1500)job.reject(new Error('音声を文字に変換できませんでした'));
        else job.resolve({text:message.text,processingMs:Number(message.processingMs)||0});
  ...
  recognize(pcm:Buffer):Promise<LocalSpeechResult> {
    ...
    return new Promise((resolve,reject)=>{
      const job={id:++this.serial,pcm,resolve,reject};
      if(!this.active)this.dispatch(job);
      else if(!this.queued)this.queued=job;
      else reject(new Error('音声認識の処理が混み合っています'));
    });
  }
