...
export function connectLocalSpeech(ws:WebSocket,recognizer:LocalRecognizer) {
  ...
  const stop=()=>{if(closed)return;closed=true;clearInterval(ticker);clearTimeout(lifetime);recognizer.release(owner);pcm.fill(0);};
  const ticker=setInterval(()=>void pump(),100);
  const lifetime=setTimeout(()=>{send({type:'ended'});ws.close(1000);stop();},20000);
  ...
  async function pump(force=false) {
    ...
    }catch(error){if(!closed){send({type:'unavailable',reason:error instanceof Error?error.message:'音声を文字に変換できませんでした'});stop();ws.close(1011);}}
  ...
  ws.on('message',(data,isBinary)=>{
    ...
    if(isBinary) {
      ...
      buffer.copy(pcm,start*2,8);lastSample=start+samples;version++;return;
    }
    ...
    if(message.type==='start'&&!started) {
      ...
      ended=true;deadline=performance.now()+650;
