...
const until = async s => { while (t < s - 1e-6) { const fine = t >= 75.4 && t < 83; const step = Math.min(fine ? 100 : 500, (s - t) * 1000); await tick(step); } };
...
await until(24.0); await shot('r1-impact');
await until(31); await fill('氷よ、壁となれ、弾き返せ');
const box = await page.locator('#magic').boundingBox();
await draw(box.x + box.width * .27, box.y + box.height * .5, 150, 170);
await until(51); await shot('r2-block');
await until(57); await fill('炎よ、集まれ、貫け');
await draw(700, 420, 200, 140);
await until(66); await shot('r3-draw'); t = await appTime();
await until(74.2); await shot('r3-charge');
await until(75.75); await shot('r3-hold');
await until(76.06); await shot('r3-pass'); await until(76.18); await shot('r3-pass2');
await until(76.9); await shot('r3-rings');
await until(77.65); await shot('r3-hit1');
await until(78.05); await shot('r3-hit3');
await until(78.8); await shot('r3-blow');
await until(79.5); await shot('r3-slow');
await until(80.2); await shot('r3-sword');
await until(81.2); await shot('r3-knee');
await until(82.6); await shot('r3-fall');
await until(85); await shot('r3-after');
await until(88.5); await shot('r3-fade');
await until(90.3); await shot('result-name');
await until(91.5); await shot('result');
