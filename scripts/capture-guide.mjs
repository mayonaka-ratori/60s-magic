...
// 先に npm run dev を起動しておく。写真は test-results/guide/ に出る。
//
...
const shot = async (name, headline = null) => {
  ...
  if (headline) await page.screenshot({ path: `${out}/${headline}.jpg`, quality: 88, type: 'jpeg',
    clip: { x: 0, y: (800 - 670) / 2, width: 1280, height: 670 } });
  await panel(false);
...
const until = async at => { while (t < at - 1e-6) { const step = Math.min(t >= s(FINISH.impact) - .5 && t < s(FINISH.handoff) ? 100 : 400, (at - t) * 1000); await tick(step); } };
const fill = async text => { await tick(200); try { await page.locator('#chant').fill(text, { timeout: 3000 }); } catch { console.log('文字を入れられなかった', t.toFixed(2)); } };
/** 円をぐるりと描く。cx, cy は画素。 */
const draw = async (cx, cy, rx, ry, turns = 24) => {
...
await next();
await until(s(FIRST.chant) + 1); await next();
...
await until(s(DEFEND.chant) - 1); await next();
