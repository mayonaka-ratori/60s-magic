...
document.querySelector<HTMLDivElement>('#app')!.innerHTML=`
  <canvas id="world" aria-hidden="true"></canvas><canvas id="magic" aria-label="手やマウスの動きで術式を描く場所"></canvas>
  ...
  <section class="welcome" id="welcome"><div class="chapter">第一幕 / 最初の魔法</div><h1><span>その手で描く。</span><span>その言葉で放つ。</span></h1><p class="intro">自由に描いた線が、ひとつの魔法になる。<br>手を動かしながら、好きな言葉を唱えてください。</p>
    ...
    <button class="primary" id="start">魔法をつくる <span class="arrow" aria-hidden="true">↗</span></button><div class="welcome-actions"><button class="text-button" id="demo">見本の動きを見る</button><button class="text-button" id="settings">接続の確認</button></div>
    <p class="notice" id="notice" role="status"></p><p class="privacy">カメラの映像はこのPC内で処理します。マイクを使う場合は、声を文字に変えるためGoogleへ送り、文字と動きの特徴をJevへ送ります。この試作では自動保存しません。</p>
...
let stage:Stage;
try {stage=new Stage(el<HTMLCanvasElement>('world'));}catch {el('loading').textContent='立体表示を準備できませんでした。Chromeの画像処理の設定を確認してください。';throw new Error('WebGL初期化に失敗');}
...
let status={jev:false,speech:false,handModel:false,model:''};
let serviceNotice='',prepareVersion=0;
const frameIntervals:number[]=[];let lastFrame=performance.now();

async function readStatus(){try{status=await fetch('/api/status').then(r=>r.json());}catch{serviceNotice='接続を確認できません。PC内の規則で遊べます。';}
  el('voice-availability').textContent=status.speech?'':'（接続の設定が必要）';el<HTMLInputElement>('use-voice').disabled=!status.speech;}
void readStatus();

function cleanup(){camera?.dispose();camera=null;voice?.dispose();voice=null;requestAbort?.abort();requestAbort=null;cursors=[];}
...
function updateUi() {
  ...
  const labels:Partial<Record<Phase,[string,string,string]>>={
    ...
    release:['発動',session.recipe?.name??'魔法を解き放つ',session.recipe?.purpose==='defend'?'あなたの壁が、騎士を押し戻す':session.recipe?.purpose==='bind'?'あなたの魔法が、騎士を捕らえる':session.recipe?.purpose==='enhance'?'術式から出た力が、騎士を押す':'あなたの術式から、敵へ'],
...
function report() {
  ...
  return {...session?.report(),mode:demo?'demo':mode,feedback,measurement:{averageFps:sorted.length?1000/(sorted.reduce((a,b)=>a+b,0)/sorted.length):null,p99FrameMs:sorted[Math.floor(sorted.length*0.99)]??null,cameraProcessingMs:camera?.latencyMs??null,note:'手を動かしてから表示されるまでの遅れは未計測。カメラ処理時間とは別。'}};
}
...
function sheet(title:string,body:string){sheetReturnFocus=document.activeElement as HTMLElement;el('sheet-title').textContent=title;el('sheet-body').replaceChildren();const pre=document.createElement('pre');pre.textContent=body;el('sheet-body').append(pre);show('sheet',true);el('sheet-close').focus();}
el('settings').addEventListener('click',async()=>{await readStatus();sheet('接続の確認',`Jev：${status.jev?'接続情報を設定済み（実通信はプレイ時）':'未設定。PC内の規則で動作'}\n音声認識：${status.speech?'Google Cloud の接続先を設定済み':'未設定。文字入力で確認できます'}\n手の認識：${status.handModel?'ファイルを準備済み':'npm run setup:assets で準備'}\n\n接続情報は .env に設定します。詳しくは README.md をご覧ください。\n\nカメラ映像は送りません。マイクを使うと声をGoogleへ送り、文字と動きの特徴をJevへ送ります。外部サービスの保存条件は各契約設定に従います。\n\nこれは最初の24秒の試作です。防御・最後の魔法・魔導書・QRは、次の段階で追加します。`);});
...
window.addEventListener('resize',()=>{stage.resize();magic.resize();});
...
function animate(now:number) {
  ...
  if(session) {
    ...
    if(current.elapsed>=14700&&!requested){
      requested=true;const state=current.freeze();requestAbort=new AbortController();const abort=requestAbort;
  ...
  stage.render(ms,session?.recipe??null,!session);
  magic.render(session?.motion.display??[],ms,session?.recipe??null,voice?.level??0,cursors,!session,stage.impactTarget);
}
requestAnimationFrame(animate);
void Promise.all([stage.scene.whenReadyAsync(),document.fonts.ready]).then(()=>show('loading',false));
