 ...
 * そのあとは一回目の確定の0.5秒前（21.5秒）まで板への転送も描画もしない（線を描いている間を軽くするため）。
...
/** 合成を見せ始める時刻（秒）。一回目の確定の0.5秒前。ここまではHTMLの層をそのまま見せ、板への転送も描画もしない。 */
 ...
 * この時刻は世界の時刻で比べるが、スローはもう終わっているので、実際の時刻でも同じ長さになる。
 */
