...
export function screenState(t: number, intensity: number, preset: EffectPreset, purpose: string | null, seed = 0): ScreenState {
  ...
  if (t >= RELEASE_AT && t < RELEASE_AT + .5) { const u = (t - RELEASE_AT) / .5; shake = shakeMax * .45 * (1 - u) * (1 - u); kickY = -shakeMax * .3 * Math.max(0, 1 - u * 6); }
