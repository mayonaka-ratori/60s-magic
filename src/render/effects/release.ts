...
export function drawRelease(f: Frame) {
  ...
  f.once('release', () => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // 持続型は粒の半分を二色目に、増幅型は白い閃光の粒に、爆発型は中間色にする。
      const pal = mix.alt && i % 2 ? mix.alt : mix.mid ? { ...pal0, spark: mix.mid } : pal0;
  ...
  if (time < 1.2) {
    glow(f, o.x, o.y, (26 + intensity * 12) * Math.max(0, 1 - time / .7), Math.max(0, 1 - time / .7) * mix.boost, pal0.main, pal0.core);
...
export function drawTravel(f: Frame) {
  ...
  if (r.form === 'wall' || r.form === 'dome' || r.form === 'wave' || (r.purpose === 'enhance' && r.count === 1)) {
    ...
    c.globalAlpha = .16 * fade; c.lineWidth = 2 + r.defense * 3; c.fillStyle = pal0.main; c.strokeStyle = pal0.main; c.beginPath();
    ...
    c.fill(); c.globalAlpha = fade * .4; c.lineWidth = (2 + r.defense * 3) * 2.2; c.stroke();
  ...
  for (let i = 0; i < r.count; i++) {
    ...
    // 持続型のときだけ、連弾は一つおきに二色目、単発は尾だけ二色目にする。
    const pal = mix.alt && r.count > 1 && i % 2 ? mix.alt : pal0;
