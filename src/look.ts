  ...
  <main class="look-page">
    ...
    <div class="look-layout" id="layout">
      <section class="look-frame" id="frame" aria-label="背景と騎士と完成した術式">
        <img class="look-backdrop" id="backdrop" src="/art/ruins-knight-v1.png" alt="石の遺跡で剣と盾を持ち、こちらを向く鎧の騎士">
        <div class="look-shade"></div>
        <div class="look-health"><span>遺跡の騎士</span><div><i></i></div></div>
        <div class="look-aim" aria-hidden="true"></div>
    ...
    <footer class="look-footer"><p id="note" role="status">背景と騎士は確認用の静止画です。術式の線と光はアプリで表示しています。</p><label>光の強さ <input id="glow" type="range" min="0" max="1.8" step="0.05" value="0.95"><output id="glow-value">標準</output></label></footer>
  </main>`;

const el = <T extends HTMLElement = HTMLElement>(id: string) => document.getElementById(id) as T;
const canvas = el<HTMLCanvasElement>('spell');
let renderer: CompletedSpell;
try { renderer = new CompletedSpell(canvas); }
...
el('restore').addEventListener('click', () => {
  ...
  el('note').textContent = '背景と騎士は確認用の静止画です。術式の線と光はアプリで表示しています。';
});
...
void backdrop.decode().then(async () => {
  ...
  redraw(); await renderer.ready();
  el('loading').hidden = true;
}).catch(() => { el('loading').textContent = '画面を読み込めませんでした。再読み込みしてください。'; });
window.addEventListener('pagehide', event => { if (!event.persisted) { observer.disconnect(); renderer.dispose(); } });
